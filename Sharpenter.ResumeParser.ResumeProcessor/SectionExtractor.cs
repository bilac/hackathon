...
namespace Sharpenter.ResumeParser.ResumeProcessor
{
    ...
    {
        private int SectionTitleNumberOfWordsLimit = 3;
        ...
        public IList<Section> ExtractFrom(IList<string> rawInput)
        {
            ...
            while (i < rawInput.Count - 1 &&
                  ( FindSectionType(rawInput[i].ToLower()) == SectionType.Unknown || FindSectionType(rawInput[i].ToLower()) == SectionType.Summary))
            {                
